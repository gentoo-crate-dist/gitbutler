...
5. BEFORE DELETION: `from_commit_traversal_tips` (explicit tips) is still
   ...
   ALSO: the ad-hoc branch-order feature (upstream 2026-07: `Overlay
   .branch_stack_orders`, `ad_hoc_branch_stack_upgrades` in `post.rs`,
   `Graph.ad_hoc_branch_stack_orders`) runs ONLY for local-branch entrypoints
   — the walk-backed non-managed path. The non-managed flip counterpart must
   reproduce it before the walk goes.
