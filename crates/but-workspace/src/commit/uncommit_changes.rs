...
/// A flat source entry for uncommitting changes from a commit.
#[derive(Debug, Clone)]
...
pub fn uncommit_changes_from_commits<'ws, 'meta, M: RefMetadata>(
    ...
    for commit_id in ordered_ids {
        let Some(group) = groups.iter().find(|group| group.commit_id == commit_id) else {
            continue;
        };
